...
public class LibDownloader
{
  ...
  private static void addPaths(String[] libs)
  {
    try
    {
      ...
      for (String lib : libs) {
        m_addURL.invoke(cl, new Object[] { new File(libDir, fileName(lib)).toURI().toURL() });
      }
    }
    catch (Exception e)
    {
  ...
  private static void download(InputStream is, int sizeGuess, File target)
    throws Exception
  {
    String name = target.getName();
    if (sizeGuess > downloadBuffer.capacity()) {
      throw new Exception(String.format("The file %s is too large to be downloaded", new Object[] { name }));
    }
    downloadBuffer.clear();
    
    int fullLength = 0;
    
    System.out.format("Downloading lib %s", new Object[] { name });
    ...
    while ((bytesRead = is.read(smallBuffer)) >= 0)
    {
      ...
      System.out.format("\rDownloading lib %s %d%%", new Object[] { name, Integer.valueOf(fullLength * 100 / sizeGuess) });
    }
