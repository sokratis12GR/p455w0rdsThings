...
import p455w0rd.p455w0rdsthings.lib.config.ConfigFile;
import p455w0rd.p455w0rdsthings.lib.config.DefaultingConfigFile;
...
public class ASMHelper
{
  ...
  private static ConfigFile loadConfig()
  {
    try
    {
      File mcDir = (File)((Object[])(Object[])Class.forName("net.minecraftforge.fml.relauncher.FMLInjectionData").getMethod("data", new Class[0]).invoke(null, new Object[0]))[6];
...
public static List<LocalVariableNode> cloneLocals(Map<LabelNode, LabelNode> labelMap, List<LocalVariableNode> locals)
  ...
  public static void dump(byte[] bytes, File file, boolean filterImportant, boolean sortLocals)
  {
    dump(new Acceptor()
    {
      public void accept(ClassVisitor cv)
      {
        new ClassReader(bytes).accept(cv, 8);
      }
    }, file, filterImportant, sortLocals);
  }
  
  public static void dump(InputStream is, File file, boolean filterImportant, boolean sortLocals)
  {
    dump(new Acceptor()
    {
      public void accept(ClassVisitor cv)
        throws IOException
      {
        new ClassReader(is).accept(cv, 8);
      }
    }, file, filterImportant, sortLocals);
  }
  
  public static void dump(ClassNode cnode, File file, boolean filterImportant, boolean sortLocals)
  {
    dump(new Acceptor()
    {
      public void accept(ClassVisitor cv)
  ...
  public static abstract interface Acceptor
  {
